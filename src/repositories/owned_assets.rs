    ...
    pub async fn list_owned_assets(&self, user_id: i64) -> sqlx::Result<Vec<OwnedAsset>> {
        sqlx::query_as::<_, OwnedAsset>(
            ...
            SELECT
             ...
             SUM(
              CASE
               WHEN o.operation_type = 'BUY' THEN (a.unit_value - o.bought_for) * o.quantity_owned
               WHEN o.operation_type = 'SELL' THEN -(a.unit_value - o.bought_for) * o.quantity_owned
              END
             ) AS value_delta,
             SUM(
              CASE
               WHEN o.operation_type = 'BUY' THEN o.quantity_owned
               WHEN o.operation_type = 'SELL' THEN -o.quantity_owned
              END
             ) AS quantity_owned,
             COALESCE(
              JSON_AGG(
...
mod tests {
    ...
    async fn test_owned_assets_buy_and_sell(db: PgPool) {
        sqlx::query("INSERT INTO users (id, username, password_hash) VALUES (1, 'alice', 'hashed')")
            .execute(&db)
            .await
            .unwrap();
        ...
        assert_eq!(owned[0].purchase_history.0[0].operation_type, AssetOperation::Buy);
